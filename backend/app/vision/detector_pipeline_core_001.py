"""
AvianVision AI — Detection & Telemetry Pipeline Module 001
...
class BoundingBox001:
    ...
    def calculate_iou(self, other: "BoundingBox001") -> float:
...
class AvianVisionDetector001:
    """
    High-throughput avian species detector 001 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox001] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox001]:
        self.frame_counter += 1
        detections: List[BoundingBox001] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox001]) -> List[BoundingBox001]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox001] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox001]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
