"""
AvianVision AI — Detection & Telemetry Pipeline Module 002
...
class BoundingBox002:
    ...
    def calculate_iou(self, other: "BoundingBox002") -> float:
...
class AvianVisionDetector002:
    """
    High-throughput avian species detector 002 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox002] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox002]:
        self.frame_counter += 1
        detections: List[BoundingBox002] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox002]) -> List[BoundingBox002]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox002] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox002]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
