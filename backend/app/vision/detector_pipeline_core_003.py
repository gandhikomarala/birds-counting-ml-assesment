"""
AvianVision AI — Detection & Telemetry Pipeline Module 003
...
class BoundingBox003:
    ...
    def calculate_iou(self, other: "BoundingBox003") -> float:
...
class AvianVisionDetector003:
    """
    High-throughput avian species detector 003 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox003] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox003]:
        self.frame_counter += 1
        detections: List[BoundingBox003] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox003]) -> List[BoundingBox003]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox003] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox003]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
