"""
AvianVision AI — Detection & Telemetry Pipeline Module 004
...
class BoundingBox004:
    ...
    def calculate_iou(self, other: "BoundingBox004") -> float:
...
class AvianVisionDetector004:
    """
    High-throughput avian species detector 004 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox004] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox004]:
        self.frame_counter += 1
        detections: List[BoundingBox004] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox004]) -> List[BoundingBox004]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox004] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox004]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
