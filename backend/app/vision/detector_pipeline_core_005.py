"""
AvianVision AI — Detection & Telemetry Pipeline Module 005
...
class BoundingBox005:
    ...
    def calculate_iou(self, other: "BoundingBox005") -> float:
...
class AvianVisionDetector005:
    """
    High-throughput avian species detector 005 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox005] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox005]:
        self.frame_counter += 1
        detections: List[BoundingBox005] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox005]) -> List[BoundingBox005]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox005] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox005]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
