"""
AvianVision AI — Detection & Telemetry Pipeline Module 006
...
class BoundingBox006:
    ...
    def calculate_iou(self, other: "BoundingBox006") -> float:
...
class AvianVisionDetector006:
    """
    High-throughput avian species detector 006 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox006] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox006]:
        self.frame_counter += 1
        detections: List[BoundingBox006] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox006]) -> List[BoundingBox006]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox006] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox006]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
