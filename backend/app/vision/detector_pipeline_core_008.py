"""
AvianVision AI — Detection & Telemetry Pipeline Module 008
...
class BoundingBox008:
    ...
    def calculate_iou(self, other: "BoundingBox008") -> float:
...
class AvianVisionDetector008:
    """
    High-throughput avian species detector 008 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox008] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox008]:
        self.frame_counter += 1
        detections: List[BoundingBox008] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox008]) -> List[BoundingBox008]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox008] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox008]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
