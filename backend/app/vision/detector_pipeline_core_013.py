"""
AvianVision AI — Detection & Telemetry Pipeline Module 013
...
class BoundingBox013:
    ...
    def calculate_iou(self, other: "BoundingBox013") -> float:
...
class AvianVisionDetector013:
    """
    High-throughput avian species detector 013 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox013] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox013]:
        self.frame_counter += 1
        detections: List[BoundingBox013] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox013]) -> List[BoundingBox013]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox013] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox013]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
