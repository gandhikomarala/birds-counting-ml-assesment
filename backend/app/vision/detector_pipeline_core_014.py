"""
AvianVision AI — Detection & Telemetry Pipeline Module 014
...
class BoundingBox014:
    ...
    def calculate_iou(self, other: "BoundingBox014") -> float:
...
class AvianVisionDetector014:
    """
    High-throughput avian species detector 014 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox014] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox014]:
        self.frame_counter += 1
        detections: List[BoundingBox014] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox014]) -> List[BoundingBox014]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox014] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox014]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
