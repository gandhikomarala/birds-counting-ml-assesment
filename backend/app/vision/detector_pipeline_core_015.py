"""
AvianVision AI — Detection & Telemetry Pipeline Module 015
...
class BoundingBox015:
    ...
    def calculate_iou(self, other: "BoundingBox015") -> float:
...
class AvianVisionDetector015:
    """
    High-throughput avian species detector 015 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox015] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox015]:
        self.frame_counter += 1
        detections: List[BoundingBox015] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox015]) -> List[BoundingBox015]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox015] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox015]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
