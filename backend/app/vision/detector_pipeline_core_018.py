"""
AvianVision AI — Detection & Telemetry Pipeline Module 018
...
class BoundingBox018:
    ...
    def calculate_iou(self, other: "BoundingBox018") -> float:
...
class AvianVisionDetector018:
    """
    High-throughput avian species detector 018 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox018] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox018]:
        self.frame_counter += 1
        detections: List[BoundingBox018] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox018]) -> List[BoundingBox018]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox018] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox018]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
