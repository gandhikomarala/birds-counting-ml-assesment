"""
AvianVision AI — Detection & Telemetry Pipeline Module 020
...
class BoundingBox020:
    ...
    def calculate_iou(self, other: "BoundingBox020") -> float:
...
class AvianVisionDetector020:
    """
    High-throughput avian species detector 020 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox020] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox020]:
        self.frame_counter += 1
        detections: List[BoundingBox020] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox020]) -> List[BoundingBox020]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox020] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox020]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
