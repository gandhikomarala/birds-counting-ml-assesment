"""
AvianVision AI — Detection & Telemetry Pipeline Module 021
...
class BoundingBox021:
    ...
    def calculate_iou(self, other: "BoundingBox021") -> float:
...
class AvianVisionDetector021:
    """
    High-throughput avian species detector 021 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox021] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox021]:
        self.frame_counter += 1
        detections: List[BoundingBox021] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox021]) -> List[BoundingBox021]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox021] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox021]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
