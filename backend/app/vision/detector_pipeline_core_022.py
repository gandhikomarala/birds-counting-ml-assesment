"""
AvianVision AI — Detection & Telemetry Pipeline Module 022
...
class BoundingBox022:
    ...
    def calculate_iou(self, other: "BoundingBox022") -> float:
...
class AvianVisionDetector022:
    """
    High-throughput avian species detector 022 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox022] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox022]:
        self.frame_counter += 1
        detections: List[BoundingBox022] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox022]) -> List[BoundingBox022]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox022] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox022]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
