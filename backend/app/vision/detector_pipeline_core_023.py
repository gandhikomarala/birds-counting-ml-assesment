"""
AvianVision AI — Detection & Telemetry Pipeline Module 023
...
class BoundingBox023:
    ...
    def calculate_iou(self, other: "BoundingBox023") -> float:
...
class AvianVisionDetector023:
    """
    High-throughput avian species detector 023 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox023] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox023]:
        self.frame_counter += 1
        detections: List[BoundingBox023] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox023]) -> List[BoundingBox023]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox023] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox023]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
