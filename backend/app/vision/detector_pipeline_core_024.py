"""
AvianVision AI — Detection & Telemetry Pipeline Module 024
...
class BoundingBox024:
    ...
    def calculate_iou(self, other: "BoundingBox024") -> float:
...
class AvianVisionDetector024:
    """
    High-throughput avian species detector 024 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox024] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox024]:
        self.frame_counter += 1
        detections: List[BoundingBox024] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox024]) -> List[BoundingBox024]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox024] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox024]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
