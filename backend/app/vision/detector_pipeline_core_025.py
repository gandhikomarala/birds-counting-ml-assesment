"""
AvianVision AI — Detection & Telemetry Pipeline Module 025
...
class BoundingBox025:
    ...
    def calculate_iou(self, other: "BoundingBox025") -> float:
...
class AvianVisionDetector025:
    """
    High-throughput avian species detector 025 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox025] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox025]:
        self.frame_counter += 1
        detections: List[BoundingBox025] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox025]) -> List[BoundingBox025]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox025] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox025]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
