"""
AvianVision AI — Detection & Telemetry Pipeline Module 026
...
class BoundingBox026:
    ...
    def calculate_iou(self, other: "BoundingBox026") -> float:
...
class AvianVisionDetector026:
    """
    High-throughput avian species detector 026 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox026] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox026]:
        self.frame_counter += 1
        detections: List[BoundingBox026] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox026]) -> List[BoundingBox026]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox026] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox026]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
