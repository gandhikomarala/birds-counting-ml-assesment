"""
AvianVision AI — Detection & Telemetry Pipeline Module 027
...
class BoundingBox027:
    ...
    def calculate_iou(self, other: "BoundingBox027") -> float:
...
class AvianVisionDetector027:
    """
    High-throughput avian species detector 027 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox027] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox027]:
        self.frame_counter += 1
        detections: List[BoundingBox027] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox027]) -> List[BoundingBox027]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox027] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox027]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
