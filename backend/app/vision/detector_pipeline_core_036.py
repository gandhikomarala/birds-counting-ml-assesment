"""
AvianVision AI — Detection & Telemetry Pipeline Module 036
...
class BoundingBox036:
    ...
    def calculate_iou(self, other: "BoundingBox036") -> float:
...
class AvianVisionDetector036:
    """
    High-throughput avian species detector 036 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox036] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox036]:
        self.frame_counter += 1
        detections: List[BoundingBox036] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox036]) -> List[BoundingBox036]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox036] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox036]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
