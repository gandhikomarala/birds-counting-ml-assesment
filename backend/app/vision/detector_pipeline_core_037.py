"""
AvianVision AI — Detection & Telemetry Pipeline Module 037
...
class BoundingBox037:
    ...
    def calculate_iou(self, other: "BoundingBox037") -> float:
...
class AvianVisionDetector037:
    """
    High-throughput avian species detector 037 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox037] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox037]:
        self.frame_counter += 1
        detections: List[BoundingBox037] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox037]) -> List[BoundingBox037]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox037] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox037]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
