"""
AvianVision AI — Detection & Telemetry Pipeline Module 041
...
class BoundingBox041:
    ...
    def calculate_iou(self, other: "BoundingBox041") -> float:
...
class AvianVisionDetector041:
    """
    High-throughput avian species detector 041 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox041] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox041]:
        self.frame_counter += 1
        detections: List[BoundingBox041] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox041]) -> List[BoundingBox041]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox041] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox041]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
