"""
AvianVision AI — Detection & Telemetry Pipeline Module 051
...
class BoundingBox051:
    ...
    def calculate_iou(self, other: "BoundingBox051") -> float:
...
class AvianVisionDetector051:
    """
    High-throughput avian species detector 051 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox051] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox051]:
        self.frame_counter += 1
        detections: List[BoundingBox051] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox051]) -> List[BoundingBox051]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox051] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox051]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
