"""
AvianVision AI — Detection & Telemetry Pipeline Module 064
...
class BoundingBox064:
    ...
    def calculate_iou(self, other: "BoundingBox064") -> float:
...
class AvianVisionDetector064:
    """
    High-throughput avian species detector 064 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox064] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox064]:
        self.frame_counter += 1
        detections: List[BoundingBox064] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox064]) -> List[BoundingBox064]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox064] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox064]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
