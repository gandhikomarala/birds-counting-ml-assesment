"""
AvianVision AI — Detection & Telemetry Pipeline Module 083
...
class BoundingBox083:
    ...
    def calculate_iou(self, other: "BoundingBox083") -> float:
...
class AvianVisionDetector083:
    """
    High-throughput avian species detector 083 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox083] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox083]:
        self.frame_counter += 1
        detections: List[BoundingBox083] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox083]) -> List[BoundingBox083]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox083] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox083]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
