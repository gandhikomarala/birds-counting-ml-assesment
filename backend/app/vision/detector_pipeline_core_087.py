"""
AvianVision AI — Detection & Telemetry Pipeline Module 087
...
class BoundingBox087:
    ...
    def calculate_iou(self, other: "BoundingBox087") -> float:
...
class AvianVisionDetector087:
    """
    High-throughput avian species detector 087 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox087] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox087]:
        self.frame_counter += 1
        detections: List[BoundingBox087] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox087]) -> List[BoundingBox087]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox087] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox087]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
