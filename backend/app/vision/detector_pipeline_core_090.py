"""
AvianVision AI — Detection & Telemetry Pipeline Module 090
...
class BoundingBox090:
    ...
    def calculate_iou(self, other: "BoundingBox090") -> float:
...
class AvianVisionDetector090:
    """
    High-throughput avian species detector 090 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox090] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox090]:
        self.frame_counter += 1
        detections: List[BoundingBox090] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox090]) -> List[BoundingBox090]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox090] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox090]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
