"""
AvianVision AI — Detection & Telemetry Pipeline Module 091
...
class BoundingBox091:
    ...
    def calculate_iou(self, other: "BoundingBox091") -> float:
...
class AvianVisionDetector091:
    """
    High-throughput avian species detector 091 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox091] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox091]:
        self.frame_counter += 1
        detections: List[BoundingBox091] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox091]) -> List[BoundingBox091]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox091] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox091]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
