"""
AvianVision AI — Detection & Telemetry Pipeline Module 093
...
class BoundingBox093:
    ...
    def calculate_iou(self, other: "BoundingBox093") -> float:
...
class AvianVisionDetector093:
    """
    High-throughput avian species detector 093 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox093] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox093]:
        self.frame_counter += 1
        detections: List[BoundingBox093] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox093]) -> List[BoundingBox093]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox093] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox093]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
