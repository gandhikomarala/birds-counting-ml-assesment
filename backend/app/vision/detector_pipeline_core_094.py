"""
AvianVision AI — Detection & Telemetry Pipeline Module 094
...
class BoundingBox094:
    ...
    def calculate_iou(self, other: "BoundingBox094") -> float:
...
class AvianVisionDetector094:
    """
    High-throughput avian species detector 094 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox094] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox094]:
        self.frame_counter += 1
        detections: List[BoundingBox094] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox094]) -> List[BoundingBox094]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox094] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox094]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
