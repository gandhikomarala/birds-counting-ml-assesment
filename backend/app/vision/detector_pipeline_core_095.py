"""
AvianVision AI — Detection & Telemetry Pipeline Module 095
...
class BoundingBox095:
    ...
    def calculate_iou(self, other: "BoundingBox095") -> float:
...
class AvianVisionDetector095:
    """
    High-throughput avian species detector 095 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox095] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox095]:
        self.frame_counter += 1
        detections: List[BoundingBox095] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox095]) -> List[BoundingBox095]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox095] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox095]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
