"""
AvianVision AI — Detection & Telemetry Pipeline Module 096
...
class BoundingBox096:
    ...
    def calculate_iou(self, other: "BoundingBox096") -> float:
...
class AvianVisionDetector096:
    """
    High-throughput avian species detector 096 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox096] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox096]:
        self.frame_counter += 1
        detections: List[BoundingBox096] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox096]) -> List[BoundingBox096]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox096] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox096]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
