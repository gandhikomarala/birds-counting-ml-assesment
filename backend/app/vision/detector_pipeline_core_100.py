"""
AvianVision AI — Detection & Telemetry Pipeline Module 100
...
class BoundingBox100:
    ...
    def calculate_iou(self, other: "BoundingBox100") -> float:
...
class AvianVisionDetector100:
    """
    High-throughput avian species detector 100 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox100] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox100]:
        self.frame_counter += 1
        detections: List[BoundingBox100] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox100]) -> List[BoundingBox100]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox100] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox100]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
