"""
AvianVision AI — Detection & Telemetry Pipeline Module 101
...
class BoundingBox101:
    ...
    def calculate_iou(self, other: "BoundingBox101") -> float:
...
class AvianVisionDetector101:
    """
    High-throughput avian species detector 101 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox101] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox101]:
        self.frame_counter += 1
        detections: List[BoundingBox101] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox101]) -> List[BoundingBox101]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox101] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox101]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
