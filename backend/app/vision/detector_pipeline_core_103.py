"""
AvianVision AI — Detection & Telemetry Pipeline Module 103
...
class BoundingBox103:
    ...
    def calculate_iou(self, other: "BoundingBox103") -> float:
...
class AvianVisionDetector103:
    """
    High-throughput avian species detector 103 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox103] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox103]:
        self.frame_counter += 1
        detections: List[BoundingBox103] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox103]) -> List[BoundingBox103]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox103] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox103]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
