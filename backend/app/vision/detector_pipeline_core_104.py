"""
AvianVision AI — Detection & Telemetry Pipeline Module 104
...
class BoundingBox104:
    ...
    def calculate_iou(self, other: "BoundingBox104") -> float:
...
class AvianVisionDetector104:
    """
    High-throughput avian species detector 104 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox104] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox104]:
        self.frame_counter += 1
        detections: List[BoundingBox104] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox104]) -> List[BoundingBox104]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox104] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox104]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
