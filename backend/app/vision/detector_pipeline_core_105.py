"""
AvianVision AI — Detection & Telemetry Pipeline Module 105
...
class BoundingBox105:
    ...
    def calculate_iou(self, other: "BoundingBox105") -> float:
...
class AvianVisionDetector105:
    """
    High-throughput avian species detector 105 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox105] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox105]:
        self.frame_counter += 1
        detections: List[BoundingBox105] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox105]) -> List[BoundingBox105]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox105] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox105]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
