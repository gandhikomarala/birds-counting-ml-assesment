"""
AvianVision AI — Detection & Telemetry Pipeline Module 106
...
class BoundingBox106:
    ...
    def calculate_iou(self, other: "BoundingBox106") -> float:
...
class AvianVisionDetector106:
    """
    High-throughput avian species detector 106 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox106] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox106]:
        self.frame_counter += 1
        detections: List[BoundingBox106] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox106]) -> List[BoundingBox106]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox106] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox106]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
