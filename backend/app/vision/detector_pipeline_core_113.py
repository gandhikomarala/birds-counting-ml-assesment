"""
AvianVision AI — Detection & Telemetry Pipeline Module 113
...
class BoundingBox113:
    ...
    def calculate_iou(self, other: "BoundingBox113") -> float:
...
class AvianVisionDetector113:
    """
    High-throughput avian species detector 113 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox113] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox113]:
        self.frame_counter += 1
        detections: List[BoundingBox113] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox113]) -> List[BoundingBox113]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox113] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox113]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
