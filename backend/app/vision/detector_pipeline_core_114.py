"""
AvianVision AI — Detection & Telemetry Pipeline Module 114
...
class BoundingBox114:
    ...
    def calculate_iou(self, other: "BoundingBox114") -> float:
...
class AvianVisionDetector114:
    """
    High-throughput avian species detector 114 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox114] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox114]:
        self.frame_counter += 1
        detections: List[BoundingBox114] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox114]) -> List[BoundingBox114]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox114] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox114]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
