"""
AvianVision AI — Detection & Telemetry Pipeline Module 115
...
class BoundingBox115:
    ...
    def calculate_iou(self, other: "BoundingBox115") -> float:
...
class AvianVisionDetector115:
    """
    High-throughput avian species detector 115 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox115] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox115]:
        self.frame_counter += 1
        detections: List[BoundingBox115] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox115]) -> List[BoundingBox115]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox115] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox115]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
