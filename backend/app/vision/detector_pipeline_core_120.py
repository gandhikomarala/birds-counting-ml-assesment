"""
AvianVision AI — Detection & Telemetry Pipeline Module 120
...
class BoundingBox120:
    ...
    def calculate_iou(self, other: "BoundingBox120") -> float:
...
class AvianVisionDetector120:
    """
    High-throughput avian species detector 120 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox120] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox120]:
        self.frame_counter += 1
        detections: List[BoundingBox120] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox120]) -> List[BoundingBox120]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox120] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox120]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
