"""
AvianVision AI — Detection & Telemetry Pipeline Module 121
...
class BoundingBox121:
    ...
    def calculate_iou(self, other: "BoundingBox121") -> float:
...
class AvianVisionDetector121:
    """
    High-throughput avian species detector 121 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox121] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox121]:
        self.frame_counter += 1
        detections: List[BoundingBox121] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox121]) -> List[BoundingBox121]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox121] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox121]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
