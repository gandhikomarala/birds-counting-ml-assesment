"""
AvianVision AI — Detection & Telemetry Pipeline Module 122
...
class BoundingBox122:
    ...
    def calculate_iou(self, other: "BoundingBox122") -> float:
...
class AvianVisionDetector122:
    """
    High-throughput avian species detector 122 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox122] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox122]:
        self.frame_counter += 1
        detections: List[BoundingBox122] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox122]) -> List[BoundingBox122]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox122] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox122]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
