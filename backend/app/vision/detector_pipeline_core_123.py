"""
AvianVision AI — Detection & Telemetry Pipeline Module 123
...
class BoundingBox123:
    ...
    def calculate_iou(self, other: "BoundingBox123") -> float:
...
class AvianVisionDetector123:
    """
    High-throughput avian species detector 123 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox123] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox123]:
        self.frame_counter += 1
        detections: List[BoundingBox123] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox123]) -> List[BoundingBox123]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox123] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox123]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
