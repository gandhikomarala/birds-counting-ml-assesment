"""
AvianVision AI — Detection & Telemetry Pipeline Module 124
...
class BoundingBox124:
    ...
    def calculate_iou(self, other: "BoundingBox124") -> float:
...
class AvianVisionDetector124:
    """
    High-throughput avian species detector 124 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox124] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox124]:
        self.frame_counter += 1
        detections: List[BoundingBox124] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox124]) -> List[BoundingBox124]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox124] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox124]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
