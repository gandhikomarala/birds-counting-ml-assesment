"""
AvianVision AI — Detection & Telemetry Pipeline Module 125
...
class BoundingBox125:
    ...
    def calculate_iou(self, other: "BoundingBox125") -> float:
...
class AvianVisionDetector125:
    """
    High-throughput avian species detector 125 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox125] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox125]:
        self.frame_counter += 1
        detections: List[BoundingBox125] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox125]) -> List[BoundingBox125]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox125] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox125]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
