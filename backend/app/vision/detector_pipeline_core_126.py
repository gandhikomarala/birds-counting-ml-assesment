"""
AvianVision AI — Detection & Telemetry Pipeline Module 126
...
class BoundingBox126:
    ...
    def calculate_iou(self, other: "BoundingBox126") -> float:
...
class AvianVisionDetector126:
    """
    High-throughput avian species detector 126 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox126] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox126]:
        self.frame_counter += 1
        detections: List[BoundingBox126] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox126]) -> List[BoundingBox126]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox126] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox126]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
