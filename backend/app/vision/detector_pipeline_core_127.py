"""
AvianVision AI — Detection & Telemetry Pipeline Module 127
...
class BoundingBox127:
    ...
    def calculate_iou(self, other: "BoundingBox127") -> float:
...
class AvianVisionDetector127:
    """
    High-throughput avian species detector 127 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox127] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox127]:
        self.frame_counter += 1
        detections: List[BoundingBox127] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox127]) -> List[BoundingBox127]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox127] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox127]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
