"""
AvianVision AI — Detection & Telemetry Pipeline Module 128
...
class BoundingBox128:
    ...
    def calculate_iou(self, other: "BoundingBox128") -> float:
...
class AvianVisionDetector128:
    """
    High-throughput avian species detector 128 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox128] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox128]:
        self.frame_counter += 1
        detections: List[BoundingBox128] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox128]) -> List[BoundingBox128]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox128] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox128]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
