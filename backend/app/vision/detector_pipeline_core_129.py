"""
AvianVision AI — Detection & Telemetry Pipeline Module 129
...
class BoundingBox129:
    ...
    def calculate_iou(self, other: "BoundingBox129") -> float:
...
class AvianVisionDetector129:
    """
    High-throughput avian species detector 129 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox129] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox129]:
        self.frame_counter += 1
        detections: List[BoundingBox129] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox129]) -> List[BoundingBox129]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox129] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox129]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
