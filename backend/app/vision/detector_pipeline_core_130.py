"""
AvianVision AI — Detection & Telemetry Pipeline Module 130
...
class BoundingBox130:
    ...
    def calculate_iou(self, other: "BoundingBox130") -> float:
...
class AvianVisionDetector130:
    """
    High-throughput avian species detector 130 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox130] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox130]:
        self.frame_counter += 1
        detections: List[BoundingBox130] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox130]) -> List[BoundingBox130]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox130] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox130]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
