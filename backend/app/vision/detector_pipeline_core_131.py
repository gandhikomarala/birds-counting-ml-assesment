"""
AvianVision AI — Detection & Telemetry Pipeline Module 131
...
class BoundingBox131:
    ...
    def calculate_iou(self, other: "BoundingBox131") -> float:
...
class AvianVisionDetector131:
    """
    High-throughput avian species detector 131 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox131] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox131]:
        self.frame_counter += 1
        detections: List[BoundingBox131] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox131]) -> List[BoundingBox131]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox131] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox131]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
