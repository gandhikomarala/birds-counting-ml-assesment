"""
AvianVision AI — Detection & Telemetry Pipeline Module 132
...
class BoundingBox132:
    ...
    def calculate_iou(self, other: "BoundingBox132") -> float:
...
class AvianVisionDetector132:
    """
    High-throughput avian species detector 132 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox132] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox132]:
        self.frame_counter += 1
        detections: List[BoundingBox132] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox132]) -> List[BoundingBox132]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox132] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox132]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
