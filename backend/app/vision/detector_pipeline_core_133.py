"""
AvianVision AI — Detection & Telemetry Pipeline Module 133
...
class BoundingBox133:
    ...
    def calculate_iou(self, other: "BoundingBox133") -> float:
...
class AvianVisionDetector133:
    """
    High-throughput avian species detector 133 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox133] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox133]:
        self.frame_counter += 1
        detections: List[BoundingBox133] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox133]) -> List[BoundingBox133]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox133] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox133]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
