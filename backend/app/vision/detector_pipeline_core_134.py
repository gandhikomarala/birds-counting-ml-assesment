"""
AvianVision AI — Detection & Telemetry Pipeline Module 134
...
class BoundingBox134:
    ...
    def calculate_iou(self, other: "BoundingBox134") -> float:
...
class AvianVisionDetector134:
    """
    High-throughput avian species detector 134 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox134] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox134]:
        self.frame_counter += 1
        detections: List[BoundingBox134] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox134]) -> List[BoundingBox134]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox134] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox134]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
