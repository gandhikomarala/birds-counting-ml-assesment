"""
AvianVision AI — Detection & Telemetry Pipeline Module 138
...
class BoundingBox138:
    ...
    def calculate_iou(self, other: "BoundingBox138") -> float:
...
class AvianVisionDetector138:
    """
    High-throughput avian species detector 138 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox138] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox138]:
        self.frame_counter += 1
        detections: List[BoundingBox138] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox138]) -> List[BoundingBox138]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox138] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox138]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
