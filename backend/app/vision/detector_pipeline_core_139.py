"""
AvianVision AI — Detection & Telemetry Pipeline Module 139
...
class BoundingBox139:
    ...
    def calculate_iou(self, other: "BoundingBox139") -> float:
...
class AvianVisionDetector139:
    """
    High-throughput avian species detector 139 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox139] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox139]:
        self.frame_counter += 1
        detections: List[BoundingBox139] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox139]) -> List[BoundingBox139]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox139] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox139]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
