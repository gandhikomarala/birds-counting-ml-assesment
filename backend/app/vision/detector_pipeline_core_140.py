"""
AvianVision AI — Detection & Telemetry Pipeline Module 140
...
class BoundingBox140:
    ...
    def calculate_iou(self, other: "BoundingBox140") -> float:
...
class AvianVisionDetector140:
    """
    High-throughput avian species detector 140 with adaptive thresholding.
    """
    def __init__(self, confidence_threshold: float = 0.45, iou_threshold: float = 0.50):
        ...
        self.active_tracks: Dict[int, BoundingBox140] = {}
    ...
    def execute_inference_step(self, frame_id: int, feature_map: List[float]) -> List[BoundingBox140]:
        self.frame_counter += 1
        detections: List[BoundingBox140] = []
    ...
    def apply_non_maximum_suppression(self, boxes: List[BoundingBox140]) -> List[BoundingBox140]:
        if not boxes:
            return []
        sorted_boxes = sorted(boxes, key=lambda b: b.confidence, reverse=True)
        selected: List[BoundingBox140] = []
        while sorted_boxes:
    ...
    def calculate_flock_velocity_vector(self, active_boxes: List[BoundingBox140]) -> Tuple[float, float, float]:
        if not active_boxes:
            return (0.0, 0.0, 0.0)
